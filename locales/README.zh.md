...
- **管理员和执行功能**：
  ...
  - **自动终止**（仅管理员可访问）：自动终止违反VRAM策略的进程（可自定义策略）。
  - **监控列表**：监控特定PID或进程名。

- **基准测试和模拟**：
  - **压力测试**：配置可配置的GEMM工作负载，用于测试热量限制和稳定性。
  - **视觉模拟**：交互式粒子物理模拟，可可视化GPU负载。
...
- **CUDA**：12.x（用于基准测试/模拟功能）。
  - *注意：如果未检测到CUDA 12.x，则基准测试/模拟功能将被禁用。*
